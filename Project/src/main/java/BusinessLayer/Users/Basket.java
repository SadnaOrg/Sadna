...
public class Basket {
    ...
    public Basket(int id, Map<Integer, Integer> products, Map<Integer, Double> prices, Map<Integer, String> categories) {
        this.shopid = id;
        this.products = new ConcurrentHashMap<>();
        this.prices = new ConcurrentHashMap<>();
        this.categories = new ConcurrentHashMap<>();
        for(int pid: products.keySet()){
            products.put(pid,products.get(pid));
            prices.put(pid, prices.get(pid));
            categories.put(pid,categories.get(pid));
        }
    }
